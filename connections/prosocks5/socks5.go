...
var (
	...
	Socks5Failed  = []byte{0x05, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x08, 0x43}
...
func GetLocalRequest(conn *net.Conn) (rawaddr []byte, host string, port string, isUdp bool, err error) {
	const (
		idVer   = 0
		idCmd   = 1
		idType  = 3 // address type index
	...
	rawaddr = buf[:reqLen]
	port = fmt.Sprintf("%d", binary.BigEndian.Uint16(buf[reqLen-2:reqLen]))
