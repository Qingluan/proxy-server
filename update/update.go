...
var (
	...
	BU       = gs.Str(`mkdir -p  /tmp/repo_update/GoR ; cd /tmp/repo_update/GoR && wget -c 'https://go.dev/dl/go1.21.11.%s-%s.tar.gz' && tar -zxvf go1.21.11.%s-%s.tar.gz ; /tmp/repo_update/GoR/go/bin/go version`)
...
func Update(beforeExit func(info string, ok bool), repo ...string) {
	repoUrl := "https://gitee.com/dark.H/proxy-z.git"
	if repo != nil {
		repoUrl = repo[0]
	...
	if res := BU.F(gs.Platform, runtime.GOARCH, gs.Platform, runtime.GOARCH).Exec(); !res.In(`go version go1.21.11`) {
